...
const createDiv = () => {
  employees.map((element) => {
    ...
    employeeName.innerText = "Name:" + element.name;
    ...
    performanceRating.innerText = `Performance:` + element.performanceRating;

    const box = document.createElement("div");
    box.className = "box";
    box.appendChild(employeeName);
    box.appendChild(position);
    box.appendChild(salary);
    box.appendChild(department);
    box.appendChild(performanceRating);
    root.appendChild(box);
  });
};
createDiv();
// function doSomething(employee) {
//   const { name, position, salary, department, performanceRating } = employee;
//   console.log({ name });
//   console.log({ salary });
// }

// doSomething(employees[0]);

BOM;
window.open;
alert;
confirm;
prompt; string butsaan number ruu hurvuulne
location
location.protocol
w3schools deploy
